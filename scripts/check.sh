...
check_python_venv() {
    PYTHON_INSTALLED=$(compgen -G '/usr/bin/python3*' || true)
    REQUIRED_VERSION="3.8"
    GOOD_PYTHON=""
    ...
    for python_interpreter in $PYTHON_INSTALLED; do
        ...
        if [ "$(printf '%s\n' "$REQUIRED_VERSION" "$PYTHON_VERSION" | sort -V | head -n1)" = "$REQUIRED_VERSION" ]; then
            ...
            GOOD_PYTHON="$GOOD_PYTHON $python_interpreter"
        fi
    done
    ...
    if [ "$GOOD_PYTHON" != "" ]; then
        ...
        for python in $GOOD_PYTHON; do
            ...
            VIRTUALENV_FOUND=0
            PIP_FOUND=0
            if $python -m venv -h 2> /dev/null | grep -qi 'usage:'; then
            ...
            if $python -m virtualenv -h 2> /dev/null | grep -qi 'usage:'; then
            ...
            if $python -m pip -h 2> /dev/null | grep -qi 'usage:'; then
...
check_disk_free_space() {
    ...
    if [ "$FREE_DISK_SPACE" -lt 120000 ]; then
        (printf >&2 "%s Warning: You appear to have less than 120GB of HDD space free on your primary partition. If you are using a separate partition, you may ignore this warning.\n" "${INFO}")
    else
...
check_ram_space() {
    ...
    if [ "$RAM_SPACE" -lt 24000000 ]; then
        (printf >&2 "%s Warning: You appear to have less than 24GB of RAM on your disk, you should consider running only a part of the lab.\n" "${INFO}")
    else
